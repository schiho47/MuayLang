...
export const client = new Client()
  // 必需：設定 API 端點（Appwrite Cloud）
  .setEndpoint('https://cloud.appwrite.io/v1')
  // 必需：你的 Project ID
  .setProject('68600758001987d2867d')

// 根據平台設定不同的 Platform ID
if (Platform.OS === 'web') {
  // Web 版本：根據當前域名設定 Platform
  if (typeof window !== 'undefined') {
    const hostname = window.location.hostname
    if (hostname === 'localhost' || hostname === '127.0.0.1') {
      client.setPlatform('localhost')
    } else if (hostname.includes('vercel.app')) {
      client.setPlatform('muaylang.vercel.app')
    } else {
      client.setPlatform(hostname)
    }
  }
} else {
  // React Native 的應用識別（需與 Appwrite 控制台的 Platform 設定一致）
