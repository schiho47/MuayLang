...
const fallbackEndpoint = isWeb
  ? DEFAULT_APPWRITE_ENDPOINT
  : 'https://api.muaylang.app/v1'
export const APPWRITE_ENDPOINT =
  process.env.NODE_ENV === 'development'
    ? PUBLIC_ENDPOINT || DEFAULT_APPWRITE_ENDPOINT
    : PUBLIC_ENDPOINT || fallbackEndpoint
...
const platformId = (() => {
  if (Platform.OS === 'web') {
    if (process.env.NODE_ENV === 'development') {
      return 'http://localhost:8081'; // Explicitly set for web development
    }
...
export const withJWTRefresh = async (operation) => {
  // Session-based auth: Appwrite SDK will manage the session token (web: localStorage, native: internal).
  // If the session expires, callers should handle the 401 (e.g. redirect to login).
